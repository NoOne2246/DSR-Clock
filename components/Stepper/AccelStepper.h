...
extern float constrainSpeed(float value, float minimum, float maximum);
...
extern void InitStepper(accelstepper_t* motor, uint8_t interface, gpio_num_t GPIOxPin1, gpio_num_t GPIOxPin2, gpio_num_t GPIOxPin3, gpio_num_t GPIOxPin4, uint8_t enable);
...
extern void InitStepperFunct(accelstepper_t* motor, void (*forward)(), void (*backward)());
...
extern void moveTo(accelstepper_t* motor, long absolute);


/// Set the target position relative to the current position
/// \param[in] relative The desired position relative to the current position. Negative is
/// anticlockwise from the current position.
extern void move(accelstepper_t* motor, long relative);
...
extern uint8_t run(accelstepper_t* motor);
...
extern uint8_t runSpeed(accelstepper_t* motor);
...
extern void setMaxSpeed(accelstepper_t* motor, float speed);

/// returns the maximum speed configured for this stepper
/// that was previously set by setMaxSpeed();
/// \return The currently configured maximum speed
extern float maxSpeed(accelstepper_t* motor);

/// Sets the acceleration/deceleration rate.
/// \param[in] acceleration The desired acceleration in steps per second
/// per second. Must be > 0.0. This is an expensive call since it requires a square
/// root to be calculated. Dont call more ofthen than needed
extern void setAcceleration(accelstepper_t* motor, float acceleration);
...
extern void setSpeed(accelstepper_t* motor, float speed);

/// The most recently set speed
/// \return the most recent speed in steps per second
extern float speed(accelstepper_t* motor);

/// The distance from the current position to the target position.
/// \return the distance from the current position to the target position
/// in steps. Positive is clockwise from the current position.
extern long distanceToGo(accelstepper_t* motor);

/// The most recently set target position.
/// \return the target position
/// in steps. Positive is clockwise from the 0 position.
extern long targetPosition(accelstepper_t* motor);

/// The currently motor position.
/// \return the current motor position
/// in steps. Positive is clockwise from the 0 position.
extern long currentPosition(accelstepper_t* motor);
...
extern void setCurrentPosition(accelstepper_t* motor, long position);

/// Moves the motor (with acceleration/deceleration)
/// to the target position and blocks until it is at
/// position. Dont use this in event loops, since it blocks.
extern void runToPosition(accelstepper_t* motor);

/// Runs at the currently selected speed until the target position is reached
/// Does not implement accelerations.
/// \return 1 if it stepped
extern uint8_t runSpeedToPosition(accelstepper_t* motor);

/// Moves the motor (with acceleration/deceleration)
/// to the new target position and blocks until it is at
/// position. Dont use this in event loops, since it blocks.
/// \param[in] position The new target position.
extern void runToNewPosition(accelstepper_t* motor, long position);

/// Sets a new target position that causes the stepper
/// to stop as quickly as possible, using the current speed and acceleration parameters.
extern void stop(accelstepper_t* motor);
...
extern void disableOutputs(accelstepper_t* motor);

/// Enable motor pin outputs by setting the motor pins to OUTPUT
/// mode. Called automatically by the constructor.
/// If the enable Pin is defined, sets it to OUTPUT mode and sets the pin to enabled.
extern void enableOutputs(accelstepper_t* motor);

/// Sets the minimum pulse width allowed by the stepper driver. The minimum practical pulse width is
/// approximately 20 microseconds. Times less than 20 microseconds
/// will usually result in 20 microseconds or so.
/// \param[in] minWidth The minimum pulse width in microseconds.
extern void setMinPulseWidth(accelstepper_t* motor, unsigned int minWidth);
...
/// \sa setPinsInverted
extern void setEnablePin(accelstepper_t* motor, gpio_num_t GPIOxEnablePin);
...
extern void setPinsInvertedStpDir(accelstepper_t* motor, uint8_t directionInvert, uint8_t stepInvert, uint8_t enableInvert);
...
/// \param[in] enableInvert    1 for inverted enable pin, 0 (default) for non-inverted
extern void setPinsInverted(accelstepper_t* motor, uint8_t pin1Invert, uint8_t pin2Invert, uint8_t pin3Invert, uint8_t pin4Invert, uint8_t enableInvert);

/// Checks to see if the motor is currently running to a target
/// \return 1 if the speed is not zero or not at the target position
extern uint8_t isRunning(accelstepper_t* motor);
...
/// move() or moveTo()
extern void computeNewSpeed(accelstepper_t* motor);
...
extern void setOutputPins(accelstepper_t* motor, uint8_t mask);

/// Called to execute a step. Only called when a new step is
/// required. Calls step1(), step2(), step4() or step8() depending on the
/// number of pins defined for the stepper.
/// \param[in] step The current step phase number (0 to 7)
extern void step(accelstepper_t* motor, long step);

/// Called to execute a step using stepper functions (pins = 0) Only called when a new step is
/// required. Calls _forward() or _backward() to perform the step
/// \param[in] step The current step phase number (0 to 7)
extern void step0(accelstepper_t* motor, long step);
...
/// \param[in] step The current step phase number (0 to 7)
extern void step1(accelstepper_t* motor, long step);

/// Called to execute a step on a 2 pin motor. Only called when a new step is
/// required. Sets or clears the outputs of pin1 and pin2.
/// \param[in] step The current step phase number (0 to 7)
extern void step2(accelstepper_t* motor, long step);
...
/// \param[in] step The current step phase number (0 to 7)
extern void step3(accelstepper_t* motor, long step);

/// Called to execute a step on a 4 pin motor. Only called when a new step is
/// required. Sets or clears the outputs of pin1, pin2, pin3, pin4.
/// \param[in] step The current step phase number (0 to 7)
extern void step4(accelstepper_t* motor, long step);

/// Called to execute a step on a 3 pin motor, such as HDD spindle. Only called when a new step is
/// required. Sets or clears the outputs of pin1, pin2, pin3.
/// \param[in] step The current step phase number (0 to 7)
extern void step6(accelstepper_t* motor, long step);

/// Called to execute a step on a 4 pin half-steper motor. Only called when a new step is
/// required. Sets or clears the outputs of pin1, pin2, pin3, pin4.
/// \param[in] step The current step phase number (0 to 7)
extern void step8(accelstepper_t* motor, long step);
